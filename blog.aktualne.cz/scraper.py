# scraper for articles and discussions in blog.aktualne.cz
...
for art in arts:
    ...
    if art_id not in existing_articles and (date - art_date) > datetime.timedelta(settings.date_distance):
        ...
        try:
            ...
            ident = sutils.create_identifier(str(article['id']))
...
with repo.git.custom_environment(GIT_COMMITTER_NAME=settings.bot_name, GIT_COMMITTER_EMAIL=settings.bot_email):
    repo.git.commit(message="happily updating blog.aktualne %s articles, %s posts, %s likes" % (str(len(new_articles)), str(len(new_posts)), str(len(new_likes))), author="%s <%s>" % (settings.bot_name, settings.bot_email))
